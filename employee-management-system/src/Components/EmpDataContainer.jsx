...
const EmpDataContainer = ({refresh, setRefresh, setIsFormOpen, setIsUpdate, setSingleEmpData}) => {
    ...
    <div className="my-3 px-3">
        <table className="w-full border-collapse border border-black">
            <thead>
                <tr>
                    {
                        tableHeader.map((theader)=> <th key={theader} className="border border-black">{theader}</th>)
                    }
                </tr>
            </thead>
            <tbody>
                            ...
                            <tr key={item.empId}>
                                {
                                    Object.keys(item).map((tdData, i)=>{
                                        return (
                                            <td className='border border-black text-center' key={i}>{item[tdData]}</td>
                                        )
                                    })
                                }
